...
export class UrlService {
  ...
  waMsg(data): Observable<string> {
    ...
    //   })
    // };
    // return this.http.post<string>('https://api.twilio.com/2010-04-01/Accounts/AC7e30d1fcd5f1e5fd6ae9e0d831e3d594/Messages.json', { formdata }, httpOptions);
    const httpOptions = {
      headers: new HttpHeaders({
        'Content-Type': 'application/json'
      })
    };
    return this.http.post<string>('http://localhost:8081', data, httpOptions);
    // return this.http.get<States[]>('http://localhost:8000');
  ...
  GetStateMasterList(): Observable<States[]> {   //GetMasterData
    return this.http.get<States[]>(this.urlHFA + "GetMasterData");
  }
  GetDisttMasterList(): Observable<States[]> {   //GetMasterData
    return this.http.get<States[]>(this.urlHFA + "GetDisttData");
  }
  GetAllDistrictList(): Observable<DistrictMaster[]> {   //GetMasterData
    return this.http.get<DistrictMaster[]>(this.urlHFA + "GetAllDisttData");
  }

  GetAllCityList(): Observable<City[]> {   //GetMasterData
